...
import { useCoverage, useInventoryOnhand, useOpenPO, useReturnAnalysis, useShipmentDaily, useStockoutRisk } from "../../api/scmApi";
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="panel-card space-y-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-orange-700">핵심 운영지표</p>
          <h3 className="mt-2 text-lg font-semibold text-gray-900">오늘 바로 봐야 하는 공급망 숫자</h3>
        </div>
      ...
      <div className="panel-card space-y-4">
        <div>
          ...
          <h3 className="mt-2 text-lg font-semibold text-gray-900">현업 판단 보조</h3>
        </div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <KpiCard title="커버리지" value={summary.coverageRate !== null ? `${(summary.coverageRate * 100).toFixed(1)}%` : null} />
        </div>
        <div className="rounded-2xl border border-black/5 bg-stone-50 px-4 py-3 text-sm text-gray-600">
          출고 {summary.shipmentCount.toLocaleString()}건, 기준 이하 품목 {summary.belowThresholdCount.toLocaleString()}건을 함께 보면서 긴급 대응 우선순위를 잡습니다.
        </div>
      </div>
    </section>
  );
